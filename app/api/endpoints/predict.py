...
async def get_player_snapshot(session, player_id, surface):
    ...
    return {
        ...
        "surf_elo": latest.w_surface_elo_before if is_w else latest.l_surface_elo_before,
        "days_off": max(0, days_off),
        "surf_days_off": max(0, surf_days_off),
        "m_win": latest.w_rolling_match_win_pct if is_w else latest.l_rolling_match_win_pct,
        "g_win": latest.w_rolling_game_win_pct if is_w else latest.l_rolling_game_win_pct,
        "sv_won": latest.w_rolling_serve_won_pct if is_w else latest.l_rolling_serve_won_pct,
        ...
        "ret_won": latest.w_rolling_return_won_pct if is_w else latest.l_rolling_return_won_pct,
        "fatigue": fatigue
    }

@router.post("/manual-predict", dependencies=[Security(get_api_key)])
async def manual_predict(data: ManualRequest):
    async with async_session() as session:
        ...
        def get_vec(p1, p2, flip=False):
            ...
            d = {
                ...
                "p1_surf_elo": p1["surf_elo"], "p2_surf_elo": p2["surf_elo"],
                "p1_days_off": p1["days_off"], "p2_days_off": p2["days_off"],
                "p1_surf_days_off": p1["surf_days_off"], "p2_surf_days_off": p2["surf_days_off"],
                "p1_m_win": p1["m_win"], "p2_m_win": p2["m_win"],
                "p1_g_win": p1["g_win"], "p2_g_win": p2["g_win"],
                "p1_sv_won": p1["sv_won"], "p1_ace_pg": p1["ace"], "p1_df_pp": p1["df"],
                "p1_bp_s": p1["bp_s"], "p1_ret_won": p1["ret_won"], "p1_fatigue": p1["fatigue"],
                "p2_sv_won": p2["sv_won"], "p2_ace_pg": p2["ace"], "p2_df_pp": p2["df"],
                "p2_bp_s": p2["bp_s"], "p2_ret_won": p2["ret_won"], "p2_fatigue": p2["fatigue"]
            }
            # Use the assembler to get embeddings and final 61-feature vector
            return assembler.assemble_manual(data.p1_id, data.p2_id, data.surface, d, flip=flip)
