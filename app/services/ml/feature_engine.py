...
class PlayerHistory:
    def __init__(self, name: str):
        self.elo = 1500.0
        self.surface_elo = {"Hard": 1500.0, "Clay": 1500.0, "Grass": 1500.0}
        self.last_match_date = None
        self.last_surface_date = {"Hard": None, "Clay": None, "Grass": None}
    ...
    def get_snapshots(self, current_date, surface):
        ...
        if not self.recent_performance:
            ...
            g_win_pct = sum(x[1] for x in self.recent_performance) / len(
                self.recent_performance
            )

        # Calculating last match date
        days_off = (
            (current_date - self.last_match_date).days if self.last_match_date else None
        )
        surf_days_off = (
        ...
        fatigue = sum(
            ...
            if (current_date - m["date"]).days <= 14
        )
        ...
        ace_pg = safe_division(tot_ace, tot_svgms, 0.0)
        bp_save = safe_division(tot_bp_s, tot_bp_f, 1.0)
...
def calculate_elo_change(
    w_elo, l_elo, k=32
):  # Standard factors and mutipliers for sports elo is being use
...
async def run_feature_engine():
    async with async_session() as session:
        ...
        for m in matches:
            ...
            m_date = (
                m.tourney_date.date()
                if isinstance(m.tourney_date, datetime)
                else m.tourney_date
            )
            ...
            update_payload.append(
                {
                    ...
                    "w_rolling_serve_won_pct": w_sv_won,
                    "l_rolling_serve_won_pct": l_sv_won,
                    "w_rolling_df_per_pt": w_df_pp,
                    "l_rolling_df_per_pt": l_df_pp,
                    "w_rolling_return_won_pct": 1.0 - l_sv_won,
                    "l_rolling_return_won_pct": 1.0 - w_sv_won,
            ...
            # Assigning last match dates
            w.last_match_date, l.last_match_date = m_date, m_date
            w.last_surface_date[m.surface], l.last_surface_date[m.surface] = (
                m_date,
                m_date,
            )
            ...
            w.recent_performance, l.recent_performance = (
                w.recent_performance[-10:],
                l.recent_performance[-10:],
            )
            w.recent_matches, l.recent_matches = (
                w.recent_matches[-20:],
                l.recent_matches[-20:],
            )
